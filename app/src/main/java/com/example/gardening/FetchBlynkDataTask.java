...
public class FetchBlynkDataTask {
    ...
    public void updateData(String urlString, UpdateDataCallback callback) //code after ma left -put updatedatacallback
    {
        ExecutorService executor = Executors.newSingleThreadExecutor();
        executor.execute(() -> {
            HttpURLConnection connection = null;
            try {
                Log.d("UPDATEDATA", "Entered updatedata ");
                URL url = new URL(urlString);
                connection = (HttpURLConnection) url.openConnection();
                connection.setRequestMethod("GET"); // Use POST to send data
                connection.connect();
                ...
                if (responseCode == HttpURLConnection.HTTP_OK) {
                    ...
                    callback.onDataUpdated(true);//code after ma left
                } else {
                    // Something went wrong with the request
                    callback.onDataUpdated(false);//code after ma left
                }
